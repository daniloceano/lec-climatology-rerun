...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="root under which results/, figures/, and docs/ are written",
    )
    ...
    if args.legacy_cache:
        environment["LEC_LEGACY_CACHE"] = str(args.legacy_cache.resolve())
    if args.legacy_results:
        environment["LEC_LEGACY_RESULTS"] = str(args.legacy_results.resolve())
    if args.output_root:
        environment["LEC_COMPARISON_OUTPUT_ROOT"] = str(args.output_root.resolve())
    ...
    for step, extra in STEPS:
        ...
        if step.startswith("step1"):
            ...
            if args.refresh:
                command += ["--refresh"]
        print(f"\n{'=' * 70}\n{' '.join([step, *extra])}\n{'=' * 70}")
        result = subprocess.run(command, cwd=BASE_DIR, env=environment)
