...
The comparison can run on an explicitly partial rerun for diagnostics, but a
final report is valid only when all 3,820 cyclones are `COMPLETE`.
...
From the repository root, in the `lec-climatology-rerun` environment, on the
server that hosts the run root, pass the external legacy inputs explicitly:

```bash
conda run --no-capture-output -n lec-climatology-rerun \
  python scripts/lec_rerun_comparison/run_all.py \
  --run-root /p1-swell/danilocs/lec_climatology_corrected_v2 \
  --legacy-cache /p1-swell/danilocs/paper_energy_patterns/data/energy_cache.parquet \
  --legacy-results /p1-swell/danilocs/paper_energy_patterns/data/temp_lec_zenodo/LEC_Results_energetic-patterns \
...
- `--legacy-cache`, `--legacy-results`, and `--output-root` remove any
  dependency on a particular checkout layout.
