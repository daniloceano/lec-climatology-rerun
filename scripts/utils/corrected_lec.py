...
Environment
-----------
``LEC_RERUN_ROOT``        override the rerun run root
``LEC_CORRECTED_DATA``    override the derived-product directory
...
def run_root() -> Path:
    ...
    return Path(os.environ.get("LEC_RERUN_ROOT", DEFAULT_RUN_ROOT))


def corrected_data_dir() -> Path:
    """Directory holding the derived corrected products read by figure scripts."""
    return Path(os.environ.get("LEC_CORRECTED_DATA", DEFAULT_CORRECTED_DATA))
