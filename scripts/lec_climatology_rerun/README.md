...
Run in the `lec-climatology-rerun` orchestration environment. Population data
remain external and are passed explicitly:

```bash
RUN=/p1-swell/danilocs/lec_climatology_corrected_v2
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.prepare \
  --run-root "$RUN" \
  --population-cache /p1-swell/danilocs/paper_energy_patterns/data/energy_cache.parquet \
...
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.pipeline \
...
`/p1-swell/danilocs/lec-climatology-rerun`, on this branch. The run root is a
...
tmux new-session -d -s lec-climatology \
  "cd /p1-swell/danilocs/lec-climatology-rerun && exec conda run --no-capture-output -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.pipeline production --run-root '$RUN' >>'$RUN/logs/scheduler.log' 2>&1"
```
...
```bash
tmux ls
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.monitor --run-root "$RUN"
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.monitor --run-root "$RUN" --watch
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.pipeline stop --run-root "$RUN"
tail -f "$RUN/logs/scheduler.log"
tail -f "$RUN/logs/"*_compute.stderr.log
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.pipeline retry-failures --run-root "$RUN"
```
...
```bash
alias lecmon='ssh swell "cd /p1-swell/danilocs/lec-climatology-rerun && conda run -n lec-climatology-rerun python scripts/lec_climatology_rerun/monitor.py --run-root /p1-swell/danilocs/lec_climatology_corrected_v2"'
```
...
```bash
RUN=/p1-swell/danilocs/lec_climatology_corrected_v2
cd /p1-swell/danilocs/lec-climatology-rerun

# 1. Phase-mean cache — input to the PCA/k-means Energy Patterns
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.build_corrected_cache \
  --run-root "$RUN" \
...
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.build_corrected_tracks \
  --run-root "$RUN" \
  --track-source /p1-swell/danilocs/paper_energy_patterns/data/tracks_SAt_filtered_with_energetics_processed.csv \
  --output /p1-swell/danilocs/paper_energy_patterns/data/corrected/tracks_with_energetics_corrected.csv

# 3. Phase-mean vertical profiles, incl. the five Ck subterms for all 3,820
conda run -n lec-climatology-rerun python -m scripts.lec_climatology_rerun.build_corrected_vertical_levels \
  --run-root "$RUN" \
  --output /p1-swell/danilocs/paper_energy_patterns/data/corrected/vertical_phase_means_corrected.parquet
```

The established commands write into the downstream article's `data/corrected/`
directory. The builders themselves and their run-root conventions live here:
...
PCA, k-means, Energy Patterns and publication figures remain outside this
repository. Review corrected products before triggering those downstream
analyses, and publish corrected artifacts as a separately versioned dataset
rather than overwriting the legacy dataset in place.
