...
def prepare(
    ...
    population_cache: Path | None = None,
) -> dict:
    config.create_directories()
    config.save()
    cache = population_cache or PROJECT_ROOT / "data" / "energy_cache.parquet"
    if not cache.is_file():
        raise FileNotFoundError(
            f"population cache not found: {cache}. Pass --population-cache explicitly."
        )
    ...
    repository_commit = git_output(Path(config.paper_repo), "rev-parse", "HEAD")
    db.set_meta("repository_commit", repository_commit)
...
def main() -> int:
    ...
    parser.add_argument(
        "--population-cache", type=Path,
        help="legacy phase-mean cache that defines the authoritative population",
    )
